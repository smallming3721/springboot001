...
public class UserServiceImpl implements UserService {
	...
	public MyResult<User> insUser(User user) {
		...
		try {
			...
			result=new MyResult<User>(ResultEnum.SUCCESS.status,"insert success",user);
		} catch (Exception e) {
			result=new MyResult<User>(ResultEnum.FAILD.status,e.getMessage());
			e.printStackTrace();
		}
		return result;
	}
	...
	public MyResult<User> updUser(User user) {
		...
		try {
			...
			result=new MyResult<User>(ResultEnum.SUCCESS.status,"update success",user);
		} catch (Exception e) {
			result=new MyResult<User>(ResultEnum.FAILD.status,e.getMessage());
			e.printStackTrace();
		}
